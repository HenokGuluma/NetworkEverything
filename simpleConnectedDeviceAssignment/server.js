...
var ARDUINO_IP_ADDRESS = '192.168.1.16';
...
const mailOptions = {
  ...
  html: '<p>You have doorbell notification from your connected devices</p>'// plain text body
};
...
function ArduinoUDPServerReceivedMessage(message, sender) {
	...
	if (message.readUInt8(1) == 0 ) {
		console.log( "received a 0");
		console.log(message.readUInt8(1));
		// Now send a message to the web browser to change color
		webSocket.emit('temperature normal', 99);
		...
		console.log( "received a 1");
		webSocket.emit('temperature high', 19);
		transporter.sendMail(mailOptions1, function (err, info) {
   		if(err)
     		console.log(err)
   		else
